class WinScene extends Phaser.Scene {
  ...
  async create(data) {
    ...
    this.add.text(CW / 2, 178, `FINAL SCORE: ${finalScore}`, {
      fontFamily: '"Press Start 2P"',
      fontSize: "12px",
      color: "#FFFF44",
    }).setOrigin(0.5);
    ...
    this.add.text(CW / 2, 228, "LEADERBOARD", {
      fontFamily: '"Press Start 2P"',
      fontSize: "10px",
      color: "#FFFFFF",
    }).setOrigin(0.5);
    for (let i = 0; i < rows.length; i++) {
      this.add.text(CW / 2, 254 + i * 28, `${i + 1}. ${rows[i].name}  ${rows[i].score}`, {
        fontFamily: '"Press Start 2P"',
        fontSize: "8px",
        color: "#BBBBBB",
      }).setOrigin(0.5);
    ...
    const playAgain = this.add.text(CW / 2, CH - 56, "PLAY AGAIN", {
      fontFamily: '"Press Start 2P"',
      fontSize: "12px",
      color: "#00FF7F",
  ...
  async loadLeaderboardRows(finalScore) {
    ...
    try {
      ...
      return json.map((r) => ({ name: String(r.name || "PAX"), score: Number(r.score || 0) })).slice(0, 5);
    } catch (e) {
      return fallback;
    }
  }
